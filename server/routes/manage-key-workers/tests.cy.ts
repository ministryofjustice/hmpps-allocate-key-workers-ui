context('Manage key workers', () => {
  ...
  it('happy path', () => {
    ...
    cy.get('.govuk-table__row').eq(5).children().eq(0).should('contain.text', 'Key-Worker, Unavailable-Inactive')
    ...
    cy.get('.govuk-table__row').eq(3).children().eq(1).should('contain.text', 'Unavailable - long term absence')
    cy.get('.govuk-table__row').eq(4).children().eq(1).should('contain.text', 'Unavailable - no prisoner contact')
    cy.get('.govuk-table__row').eq(5).children().eq(1).should('contain.text', 'Inactive')
    ...
    cy.get('.govuk-table__row').eq(3).children().eq(3).should('contain.text', '4')
    cy.get('.govuk-table__row').eq(4).children().eq(3).should('contain.text', '12')
    cy.get('.govuk-table__row').eq(5).children().eq(3).should('contain.text', '11')
    ...
    cy.get('[aria-sort="none"] > a').eq(1).click()
